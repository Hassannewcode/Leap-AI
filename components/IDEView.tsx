...
const IDEView: React.FC<IDEViewProps> = ({ activeWorkspace, isLoading, isCreatingAsset, loadingMode, aiProgress, isOnline, onGenerate, onPositiveFeedback, onRetry, onRestoreCheckpoint, onRenameWorkspace, onDeleteWorkspace, onReturnToLauncher, onUpdateFileContent, onUploadLocalAssets, onCreateLocalAsset, onUndo, onRedo, canUndo, canRedo }) => {
    ...
    return (
        <div className="flex h-screen w-screen bg-black text-gray-300 font-sans">
            ...
            {isLoading && (
                <div className="absolute bottom-5 left-1/2 -translate-x-1/2 bg-gray-900/80 backdrop-blur-sm border border-white/10 text-gray-200 px-4 py-3 rounded-lg flex items-start gap-4 shadow-lg z-50 transition-opacity duration-300 animate-fade-in w-full max-w-xl">
                    <SpinnerIcon className="w-5 h-5 text-blue-400 flex-shrink-0 mt-0.5" />
                    <div className="text-left overflow-hidden flex-1">
                        <p className="text-sm font-medium whitespace-pre-wrap break-words max-h-32 overflow-y-auto">
                           {aiProgress || "AI is thinking..."}
                        </p>
                        <p className="text-xs text-gray-400 font-mono tracking-tighter mt-1">Elapsed: {elapsedTime}s</p>
                    </div>
                </div>
            )}
        </div>
